...
- Run: `go run main.go`
- Integrates with existing GCS: Replaces/extends mavlink-routerd; forwards to QGC/gimbal UIs while bridging to NATS.
...
## 🛠️ Built For
Self-hosted edge deployments: Run on GCS laptops for tactical awareness. Integrates MAVLink telemetry, video streams, and detections into a unified NATS fabric for real-time C2 and analytics.
...
```mermaid
flowchart LR
    subgraph "Vehicle/Edge Device"
        Autopilot[["Autopilot (PX4/ArduPilot)"]]
        Camera[("Camera (USB/RTSP)")]
    end

    subgraph "GCS Laptop/Edge Node"
